...
raw_encuesta$encuesta_calidad.barrio %>% unique() %>% sort()
...
```{r}
k_maximo <- 15
datos_agrupamiento <- db_escolaridad_escalado %>% dplyr::select(-n, -encuesta_calidad.barrio, -encuesta_calidad.comuna)
...
grupos_mapa <- merge(x = grupos_barrios, y = nombres_mapa, by.x = "barrio_nombre", by.y = "nombre_barrio")
grupos_mapa <- grupos_mapa %>% mutate(grupo = as.factor(grupo))

factpal <- colorFactor(rainbow(k_optimo_kmeans), grupos_mapa$grupo)

leaflet(data = political) %>% 
  addTiles() %>% 
  addProviderTiles(providers$OpenStreetMap) %>% 
  addPolygons(fill = TRUE, stroke = TRUE, weight = 2, color = ~factpal(grupos_mapa$grupo), 
...
db_escolaridad$encuesta_calidad.barrio %>% unique()
political$NOMBRE %>% unique()
```
...
```{r}
agrupamiento_kmeans$cluster
```

```{r}
interseccion <- intersect(db_escolaridad$encuesta_calidad.barrio, political$NOMBRE)

db_escolaridad$encuesta_calidad.barrio[!(db_escolaridad$encuesta_calidad.barrio %in% interseccion)]
```
