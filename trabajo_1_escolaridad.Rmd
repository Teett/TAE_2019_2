...
```{r}
agrupamiento_kmeans <- kmeans(db_escolaridad_escalado, centers = 7, nstart = 10, iter.max = 15 )
```
...
nombres_mapa <- data.frame(political$NOMBRE)
rownames(nombres_mapa) <- political$NOMBRE
clusters_y_nombres <- data.frame(agrupamiento_kmeans$cluster)
...
leaflet(data = political) %>% 
  ...
  addPolygons(fill = TRUE, stroke = TRUE, weight = 2, color = colores, 
...
## Mapa geográfico usando el agrupamiento (K-means)
